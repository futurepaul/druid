...
use piet_common::{Color, Piet, RenderContext, Text, TextAttribute, TextLayoutBuilder};
...
impl WinHandler for PerfTest {
    ...
    fn paint(&mut self, piet: &mut Piet, _: Rect) -> bool {
        ...
        let font = piet.text().font_family("Consolas").unwrap_or_default();
        let small_size = 15.;
        let large_size = 48.;
        ...
        let layout = piet
            .text()
            .new_text_layout(&msg)
            .font(font.clone(), small_size)
            .default_attribute(TextAttribute::ForegroundColor(FG_COLOR))
            .build()
            .unwrap();
