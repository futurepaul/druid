...
use crate::piet::{
    FontFamily, PietTextLayout, RenderContext, Text, TextAttribute, TextLayout, TextLayoutBuilder,
};
...
impl<T: Data, W: Widget<T>> WidgetPod<T, W> {
    ...
    fn make_widget_id_layout_if_needed(&mut self, id: WidgetId, ctx: &mut PaintCtx, env: &Env) {
        if self.debug_widget_text.is_none() {
            let debug_color = env.get_debug_color(ctx.widget_id().to_raw());
            let (r, g, b, _) = debug_color.as_rgba8();
    ...
    fn debug_paint_widget_ids(&self, ctx: &mut PaintCtx, env: &Env) {
        ...
        if let Some(text) = text {
            let text_size = Size::new(text.size().width, text.size().height);
